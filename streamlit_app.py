...
with st.sidebar:
    ...
    display_mode = st.radio("Display mode", ["Overlay (compare on same axes)", "Stacked (one panel per file)"],
                            index=0 if qp_get_str("mode", "overlay").lower() == "overlay" else 1)

    x_mode = st.radio("X-axis", ["Timestamp", "Relative time (seconds from file start)"],
    ...
    band_abs = st.number_input("Absolute band (same units as signal)", value=qp_get_float("db_abs", 0.02),
                               step=0.01, format="%.5f")
    band_pct = st.number_input("Percentage band (%)", value=qp_get_float("db_pct", 0.0),
                               step=0.1, format="%.3f") / 100.0
    ...
    computed_channels = st.multiselect("Add computed channels",
...
def load_one(file_bytes: bytes, name: str) -> pd.DataFrame:
    """Load CSV; if pyarrow is present, persist a small parquet cache keyed by file content hash."""
    df = None
...
if x_mode == "Timestamp":
    ...
    r0, r1 = st.slider("Time range (relative seconds from each file start)",
...
def process_one(df: pd.DataFrame, name: str) -> pd.DataFrame:
    ...
    if x_mode == "Timestamp":
        ...
        # NEW: convert relative seconds to pseudo-datetime for ms tick formatting
        #      (1970-01-01 + seconds), which lets Plotly use date tick formats
...
def apply_legend_position(fig: "go.Figure", where: str):
    ...
    if where == "Below":
        fig.update_layout(legend=dict(orientation="h", yanchor="top", y=-0.18, xanchor="left", x=0.0,
                                      itemsizing="trace", itemwidth=60, font=dict(size=11)))
        # bump bottom margin safely
        try:
...
# ──────────────────────────────────────────────────────────────────────────────
# Figure builders (now using ms formatting on both Timestamp & Relative)
# ──────────────────────────────────────────────────────────────────────────────
...
def make_stacked_figure(df: pd.DataFrame) -> "go.Figure":
    ...
    for i, source in enumerate(sources, start=1):
        ...
        for y in y_cols_display:
            fig.add_trace(go.Scatter(x=g[xk], y=g[y], mode="lines",
                                     name=f"{pretty_name(source)} · {y}",
                                     line=dict(width=1), opacity=1.0), row=i, col=1)
            if show_extrema: add_extrema_markers(fig, g, xk, y, source)
    ...
    fig.update_layout(
        ...
        showlegend=True,
        template="plotly" if st.get_option("theme.base") == "light" else "plotly_dark",
        xaxis=xaxis_time_layout(),  # first subplot's xaxis config; Plotly shares it to others
    )
    apply_legend_position(fig, legend_location)
